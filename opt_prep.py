"""OPT 2025 parte A: prepara arrays flujo+Bayes+sesion y guarda npz.
Uso: py opt_prep.py
"""
import sys
...
def main():
    ...
    atr_med = atr.rolling(100).mean().bfill()
    ...
    hora = d["time"].dt.hour.values
    ses = ((hora >= 7) & (hora <= 20)).astype(int)
    exp = (atr.values > atr_med.values).astype(int)
    np.savez_compressed(DATA / "opt_2025.npz", hi=d["high"].values, lo=d["low"].values,
                        cl=d["close"].values, z=z.values, p=p, atr=atr.values,
                        ses=ses, exp=exp, t=d["time"].astype(str).values)
    print(f"OPT PREP OK n={len(d)} -> data/opt_2025.npz", flush=True)
